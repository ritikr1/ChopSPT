...
def train_model(token_folder, vocab_size=512, epochs=113, batch_size=16, seq_len=512):
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        for chunk_idx, file_chunk in enumerate(file_chunks):
            ...
            model.train()
            total_loss = 0
            for xb, yb, fnames in tqdm(dataloader, desc=f"Epoch {epoch_counter+1}"):
                xb, yb = xb.to(device), yb.to(device)
                logits = model(xb)
                if torch.any((yb < 0) | (yb >= vocab_size)):
                    print("Invalid target detected.")
                    for b_idx in range(yb.size(0)):
                        bad_tokens = yb[b_idx][(yb[b_idx] < 0) | (yb[b_idx] >= vocab_size)]
                        if len(bad_tokens) > 0:
                            print(f"From file: {fnames[b_idx]}")
                            print("Offending tokens:", bad_tokens.tolist())
